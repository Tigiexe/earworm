...
const MP = {
  ...
  pub() { return Object.values(this.players).map(p => ({ id: p.id, name: p.name, score: p.score, count: p.count, include: p.include, connected: p.connected, answered: !!this.answers[p.id], streak: p.streak, correct: p.correct })); },
  ...
  async hostGame() {
    ...
    const mine = Lib.all();
    this.players = { host: { id: 'host', name, score: 0, streak: 0, correct: 0, include: true, count: mine.length, connected: true } };
  ...
  hostRecv(id, d) {
    ...
    if (d.t === 'hello') {
      ...
      const name = String(d.name || '').replace(/\s+/g, ' ').trim().slice(0, 20) || 'Guest';
      ...
      this.players[id] = old ? { ...old, id, name, count: tracks.length, connected: true } : { id, key, name, score: 0, streak: 0, correct: 0, include: true, count: tracks.length, connected: true };
      this.libs[id] = tracks;
  ...
  renderLobby() {
    ...
    const mixCount = isHost ? dedupe(Object.entries(this.libs).filter(([id]) => this.players[id]?.include && this.players[id]?.connected).flatMap(([, l]) => l)).length : null;
    UI.show('mp');
    $('#scr-mp').innerHTML = `
      <div class="mp-grid">
        <div class="panel">
        ...
        <div class="panel"><h2>Players</h2>
          ${players.map(p => `<div class="player"><span class="dot ${p.connected ? 'on' : ''}"></span><span class="n"><b>${esc(p.name)}</b>${p.id === this.myId ? ' <span class="mute">(you)</span>' : ''}<br><small class="mute">${p.count ? fmtN(p.count) + ' songs' : 'no songs shared'}</small></span>
            ${isHost && p.count ? `<label class="chk" style="padding:0"><input type="checkbox" data-act="mpInclude" data-id="${esc(p.id)}" ${p.include ? 'checked' : ''}> Mix in</label>` : ''}</div>`).join('')}
  ...
  startGame() {
    ...
    // balance by player, and only use songs everyone can hear (most guests get 30-second previews)
    const types = Engine.setup(pool, { groupBy: t => t.owners || [], needPreview: true });
  ...
  onReveal(msg) {
    ...
    if (me) { $('#hudScore').textContent = fmtN(me.score); $('#hudStreak').textContent = streakTxt(me.streak); }
    const table = `<div style="margin-top:12px">${sorted.map((p, i) => { const r = msg.results[p.id]; return `<div class="board-row"><span class="pos">${i + 1}</span><span class="n">${esc(p.name)}${r?.given ? ` <small class="mute">“${esc(r.given)}”</small>` : ''}</span><span class="${r?.correct ? 'ok' : 'no'} num">${r?.pts > 0 ? '+' : ''}${esc(r?.pts ?? 0)}</span><b class="num">${fmtN(p.score)}</b></div>`; }).join('')}</div>`;
  ...
  renderBoard(players) {
    ...
    b.innerHTML = `<h3>Scores</h3>${sorted.map((p, i) => `<div class="board-row"><span class="pos">${i + 1}</span><span class="dot ${p.answered ? 'on' : ''}" title="${p.answered ? 'Answered' : 'Thinking'}"></span><span class="n">${esc(p.name)}${p.id === this.myId ? ' <small class="mute">(you)</small>' : ''}${p.connected ? '' : ' <small class="mute">(left)</small>'}</span><b class="num">${fmtN(p.score)}</b></div>`).join('')}`;
    const me = players.find(p => p.id === this.myId); if (me) $('#hudScore').textContent = fmtN(me.score);
  },
  showPodium(players) {
    ...
    const medal = ['🥇', '🥈', '🥉'];
    $('#scr-results').innerHTML = `<div class="res-head"><p class="mute">Final scores</p><div class="res-score" style="font-size:clamp(46px,9vw,90px)">${esc(sorted[0]?.name || '')} wins</div></div>
      <div class="podium panel">${sorted.map((p, i) => `<div class="board-row" style="font-size:18px"><span class="pos">${medal[i] || i + 1}</span><span class="n"><b>${esc(p.name)}</b>${p.id === this.myId ? ' <small class="mute">(you)</small>' : ''}<br><small class="mute">${fmtN(p.correct)} correct</small></span><b class="num">${fmtN(p.score)}</b></div>`).join('')}
  ...
  reset() {
    ...
    Object.assign(this, { conns: new Set(), players: {}, libs: {}, answers: {}, curQ: null, round: 0, total: 0, state: 'idle', lobby: null, code: null, myId: null });
  },
  leave(silent = false) {
